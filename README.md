...
localização atual do celular junto com os dados do medicamento. A API guarda
esse "registro" e permite consultá-lo depois.
...
```
AppRecife-backend/
|- data/                     # registros.json e gerado aqui em tempo de execucao
|- src/
|  |- config/
|  |  \- env.js              # leitura de variaveis de ambiente (porta)
|  |- controllers/
|  |  \- registrosController.js  # logica de cada rota
|  |- database/
|  |  \- db.js               # leitura/escrita no arquivo JSON
|  |- models/
|  |  \- registro.js         # validacao e formato de um registro
|  |- routes/
...
> Para que o celular alcance o backend durante a demonstração, o app usa o IP
> da máquina na rede local automaticamente (via Expo). Garanta que o PC e o
...
A consulta a esse conjunto é feita pelo aplicativo (frontend) via API CKAN
`datastore_search`; o backend apenas guarda o que o usuário decide salvar.
